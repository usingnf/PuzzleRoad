...
public class Boss_Lazer : MonoBehaviour
{
    ...
    [SerializeField] private GameObject obj_hit;
    [SerializeField] private GameObject obj_flash;
    ...
    void Update()
    {
        ...
        if (Laser != null && UpdateSaver == false)
        {
            ...
            if (Physics.Raycast(transform.position, target.position - this.transform.position, out hit, MaxLength, LayerMask.GetMask("Default", "Ground", "Player")))//CHANGE THIS IF YOU WANT TO USE LASERRS IN 2D: if (hit.collider != null)
            {
            ...
            else
            {
                ...
                var EndPos = transform.position + (target.position - this.transform.position).normalized * MaxLength;
